...
const fancy = document.querySelectorAll('#fancy-inputs input[type="text"] ');
const icons = document.querySelectorAll('#fancy-inputs label.input::before');
...
fancy.forEach(item => {
  item.addEventListener('blur', (evt) => {
    ...
    if (target.value.length > 0) {
      ...
      target.nextSibling.nextSibling.textContent = "Необходимо ввести данные";
    }
  });
});
;
...
let mySecondSlider = new Swiper('.faces-slist__wrapper', {
  // effect: 'slide',
  slidesPerView: 3,
  watchOverflow: true,
  autoplay: {
